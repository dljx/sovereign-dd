...
def test_factor_stamp_carries_smart_money_tilt():
    """Capital-flow smart-money tilt (2026-07-12) — measurement-only, does not
    itself bump v (it just rides whatever v the latest selection-affecting
    change stamped)."""
    from upload_kv import _factor_stamp
    d = {"technicals": {}, "financials": {}, "capital_flow": {"smart_tilt": 4.2e8}}
    stamped = _factor_stamp(d, {})
    assert stamped["cap_smart_tilt"] == 4.2e8 and stamped["v"] == 5
